...
- `.github/workflows/sonarqube.yml`: runs SonarQube Community analysis on `main` and pull requests when `SONAR_TOKEN` and `SONAR_HOST_URL` secrets are configured.
...
SonarQube analysis is opt-in and stays non-blocking until the required secrets are configured.

- Set `SONAR_HOST_URL` to your SonarQube Community Edition server URL.
- Set `SONAR_TOKEN` to a project-scoped token with analysis permissions.
- The Gradle `sonar` task consumes JaCoCo XML coverage, JUnit test results, and the Detekt XML report.

Example repository secret setup with GitHub CLI:

```bash
gh secret set SONAR_HOST_URL --repo bertramkranz/PersonalAgent --body "https://your-sonarqube-host"
gh secret set SONAR_TOKEN --repo bertramkranz/PersonalAgent --body "your-sonar-token"
```

After secrets are configured, trigger the workflow manually once to validate end-to-end analysis:

```bash
gh workflow run sonarqube.yml --repo bertramkranz/PersonalAgent
```

If you decide SonarQube should become a required branch-protection check after validation, update required contexts to include `sonarqube` in `.github/workflows/bootstrap-repo-guardrails.yml` and re-run the bootstrap workflow.
