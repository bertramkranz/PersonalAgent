...
- `.github/workflows/code-maat-hotspots.yml`: runs a monthly Code Maat repository-mining report for churn, coupling, authorship, and summary metrics.
- `.github/workflows/error-log-review.yml`: runs a weekly scan of recent CI `gradle-reports` artifacts, summarises JUnit test failure patterns, and publishes a markdown report artifact.
- `.github/workflows/secret-scan.yml`: scans pushes and pull requests for leaked secrets and uploads SARIF findings.

CI report artifacts include Gradle reports under `build/reports/**`, including JaCoCo coverage output generated by `jacocoTestReport`.

The monthly Code Maat workflow uploads repository-mining CSV reports and a short markdown summary under the `code-maat-hotspots` artifact.
...
Download the latest artifact from the **Error Log Review** workflow run:

```bash
gh run list --workflow error-log-review.yml --repo bertramkranz/PersonalAgent --limit 1
gh run download <run-id> --name error-log-review-<run-id>
```

The `README.md` inside the artifact contains:
...
- **High-count patterns** — Likely persistent or flaky tests. Investigate root cause or open a tracking issue.
- **Recent failures** (high run ID) — May indicate a regression; compare commits between the last-green run and the failing one.
- **`error` kind** — Unexpected exception thrown; look for infrastructure or test-setup problems.
- **`failure` kind** — Assertion failure; compare the expected vs. actual value in the test output.
...
SonarQube Community analysis in this repository is local-only.

- Start SonarQube locally with [../docker-compose.sonarqube.yml](../docker-compose.sonarqube.yml).
- Use `SONAR_HOST_URL=http://localhost:9000` and a locally generated `SONAR_TOKEN`.
- Run analysis from your workstation with `./gradlew --no-daemon check sonar`.

There is intentionally no GitHub Actions SonarQube workflow and no SonarQube branch-protection required check.
