...
def build_report(run_dirs, all_records):
    ...
    if not sorted_groups:
        ...
        for fp, records in sorted_groups[:20]:
            count = len(records)
            most_recent_record = max(records, key=lambda r: run_id_sort_key(r["run_id"]))
            most_recent_run = most_recent_record["run_id"]
            kinds = sorted(set(r["kind"] for r in records))
            sample_msg = most_recent_record["message"] or "(no message)"
            sample_display = sample_msg[:200] + ("…" if len(sample_msg) > 200 else "")
            sample_display = sample_display.replace("|", "\\|").replace("\n", " ").replace("\r", "")
    ...
    lines += [
        ...
        "",
        "_This report is auto-generated by [`.github/workflows/error-log-review.yml`](https://github.com/bertramkranz/PersonalAgent/blob/main/.github/workflows/error-log-review.yml)._",
    ]
